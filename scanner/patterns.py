...
PATTERNS: list[Pattern] = [
    ...
    _p("Supabase API Key",         r"sbp_[a-f0-9]{40}",                       "HIGH", "Supabase project API key"),
    _p("Vercel Token",             r"vercel_[A-Za-z0-9]{24,}",                "HIGH", "Vercel access token"),
    _p("Cloudflare Global Key",
       r"(?i)cloudflare(?:.{0,20})?['\"]([0-9a-f]{37})['\"]",
       "HIGH", "Cloudflare global API key"),
    _p("Datadog API Key",
       r"(?i)(?:dd|datadog)(?:_|-)?api(?:_|-)?key\s*[=:]\s*['\"]?([a-f0-9]{32})['\"]?",
       "HIGH", "Datadog API key"),
    _p("Datadog App Key",
       r"(?i)(?:dd|datadog)(?:_|-)?app(?:_|-)?key\s*[=:]\s*['\"]?([a-f0-9]{40})['\"]?",
       "HIGH", "Datadog application key"),
    _p("PlanetScale Token",        r"pscale_tkn_[A-Za-z0-9_-]{32,}",          "HIGH", "PlanetScale database token"),
    _p("PlanetScale OAuth Token",  r"pscale_oauth_[A-Za-z0-9_-]{32,}",        "HIGH", "PlanetScale OAuth token"),
    _p("PlanetScale Password",     r"pscale_pw_[A-Za-z0-9_-]{32,}",           "HIGH", "PlanetScale password"),
    _p("Postman API Key",          r"PMAK-[A-Za-z0-9]{24}-[A-Za-z0-9]{34}",   "HIGH", "Postman API key"),
    _p("Grafana Service Token",    r"glsa_[A-Za-z0-9]{32}_[A-Fa-f0-9]{8}",    "HIGH", "Grafana service account token"),
    _p("Grafana Cloud Token",      r"glc_[A-Za-z0-9+/]{32,}={0,2}",           "HIGH", "Grafana Cloud API token"),
    _p("Sentry Auth Token",        r"sntrys_[A-Za-z0-9]{38,}",                "HIGH", "Sentry auth token"),
    _p("Sentry DSN",
       r"https://[a-f0-9]{32}@[a-z0-9.-]+\.ingest\.sentry\.io/[0-9]+",
       "MEDIUM", "Sentry DSN (may contain private key)"),
    _p("Doppler Token",            r"dp\.st\.[A-Za-z0-9_-]{40,}",             "HIGH", "Doppler service token"),
    _p("Age Secret Key",           r"AGE-SECRET-KEY-1[QPZRY9X8GF2TVDW0S3JN54KHCE6MUA7L]{58}", "CRITICAL", "Age encryption secret key"),
    _p("Infisical Token",          r"st\.[a-f0-9]{8}\.[a-f0-9]{32}\.[a-f0-9]{16}", "HIGH", "Infisical service token"),
    _p("Finicity App Key",         r"(?i)finicity(?:.{0,20})?['\"]([a-f0-9]{32})['\"]", "HIGH", "Finicity application key"),
    _p("Flutterwave Secret Key",   r"FLWSECK-[a-f0-9]{32}-X",                 "HIGH", "Flutterwave secret key"),
    _p("Coinbase Access Token",    r"(?i)coinbase(?:.{0,20})?['\"]([A-Za-z0-9]{64})['\"]", "HIGH", "Coinbase API access token"),
    _p("Twitch Client Secret",
